  ...
  useEffect(() => {
    if (isEdit) {
      setNote({
        ...
        removed_at: currentNote.removed_at,
        removed_in_ui: currentNote.removed_in_ui,
        recovered_at: currentNote.recovered_at,
        temp_note_id: currentNote.temp_note_id,
        update_loading: currentNote.update_loading,
      }); // при нажатии на кнопку готовая изменённая заметка приобретает все те же параметры, которые были у редактируемой.
    } else {
